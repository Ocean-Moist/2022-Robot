...
public class ShootOneBall extends SequentialCommandGroup {
    public ShootOneBall(DrivetrainSubsystem drivetrain, IndexerSubsystem indexer, IntakeSubsystem intake,
                        LimelightSubsystem limelight, OuttakeSubsystem outtake, ShooterDataTable shooterDataTable) {
        ...
        try {
            addCommands(
                    ...
                    new SetHoodAngle(outtake,
                            shooterDataTable.getSpecs(limelight.getLimelightOutputAtIndex(0)).getAngle()),
                    new SetTurretActive(outtake, true),
                    new SetSpecificShooterPower(outtake,
                            shooterDataTable.getSpecs(limelight.getLimelightOutputAtIndex(0)).getPower()),
